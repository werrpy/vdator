        ...
        if has(self.mediainfo, "general.0.tmdb"):
            ...
            try:
                ...
                if is_movie:
                    ...
                    matched["tmdb_title"] = (
                        "original_title" in tmdb_info
                        and movie_data["name"] == tmdb_info["original_title"]
                    )
            ...
            if matched["title"] and matched["year"]:
                ...
                if matched["title"]:
                    ...
                    if tmdb_info and "original_title" in tmdb_info:
                        reply += self.reporter.print_report(
                            "error", "TMDb: Name: `" + tmdb_info["original_title"] + "`"
                        )
                        reply += show_diff(
                            tmdb_info["original_title"], movie_data["name"]
                        )
                        matched["title_replied"] = True
                ...
                if matched["year"]:
                    ...
                    if imdb_movie and "year" in imdb_movie:
                        reply += self.reporter.print_report(
                            "error", "IMDb: Year: `" + imdb_movie["year"] + "`"
                        )
                        matched["year_replied"] = True
                    if tmdb_year:
                        reply += self.reporter.print_report(
                            "error", "TMDb: Year: `" + tmdb_year + "`"
                        )
                        matched["year_replied"] = True
                    ...
                    if len(self.mediainfo["menu"][i]) > 0:
                        for j, item in enumerate(self.mediainfo["menu"][i]):
                            if "language" in self.mediainfo["menu"][i][j]:
                                try:
                                    ch_lang = iso639_languages.get(
                                        alpha2=self.mediainfo["menu"][i][j]["language"]
                                    )
                        ...
                        try:
                            ...
                            if ch_lang in chapter_langs:
                                ...
                                chapter_langs_names = ", ".join(
                                    list(set([lang.name for lang in chapter_langs]))
                                )
