...
class Checker():
  ...
  def print_report(self, type, content):
  ...
  def check_muxing_mode(self):
    ...
    reply += self._check_muxing_mode_section('general')
    reply += self._check_muxing_mode_section('video')
    reply += self._check_muxing_mode_section('audio')
    reply += self._check_muxing_mode_section('text')
          ...
          if bdinfo_audio_parts[audio_split_index] == "DTS-HD Master Audio" and \
            ...
            if 'title' in self.mediainfo['audio'][i]:
              if title in self.mediainfo['audio'][i]['title']:
                reply += self.print_report("correct", "Audio " + self._audio_id(i) + ": Track names match\n")
              else:
                ...
                if is_bad_audio_format:
                  reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Bad conversion:\n```fix\nBDInfo: " + title + "\nMediaInfo: " + self.mediainfo['audio'][i]['title'] + "```")
                else:
                  reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Track names mismatch:\n```fix\nBDInfo: " + title + "\nMediaInfo: " + self.mediainfo['audio'][i]['title'] + "```")
            else:
              reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Missing track name\n")
    else:
        ...
        if bdinfo_audio_format == 'Dolby Digital Audio':
          if 'format' in self.mediainfo['audio'][i]:
            if self.mediainfo['audio'][i]['format'] == 'AC-3':
              reply += self.print_report("correct", "Audio " + self._audio_id(i) + ": Commentary already AC-3\n")
            else:
              reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Commentary should be AC-3 instead of " + self.mediainfo['audio'][i]['format'] + "\n")
          else:
            reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Commentary does not have a format\n")
            
          return is_commentary, reply
      else:
        reply += self.print_report("warning", "Audio #" + self._audio_id(i) + ": Cannot verify commentary audio conversion\n")
        return is_commentary, reply
      ...
      if 'format' in self.mediainfo['audio'][i] and self.mediainfo['audio'][i]['format'] == 'AC-3':
        if 'bit_rate' in self.mediainfo['audio'][i]:
          if '224' in self.mediainfo['audio'][i]['bit_rate']:
            reply += self.print_report("correct", "Audio " + self._audio_id(i) + ": Commentary converted to AC-3 @ 224 kbps\n")
          else:
            reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Commentary AC-3 bitrate should be 224 kbps instead of " + self.mediainfo['audio'][i]['bit_rate'] + "\n")
        else:
          reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Commentary AC-3 does not have a bitrate\n")
      else:
        reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Commentary should be converted to AC-3\n")
  ...
  def _check_audio_conversion(self, i, bdinfo, mediainfo, audio_from, audio_to):
    ...
    if ' / ' not in bdinfo['audio'][i] or \
      ...
      reply += self.print_report("warning", "Could not verify audio " + self._audio_id(i))
      return reply
    ...
    if mediainfo_parts[0] == audio_to:
      if (mediainfo_parts[1] != bdinfo_audio_parts[2]):
        reply += self.print_report("error", "Audio " + self._audio_id(i) + ": Channel Mismatch, " + audio_from + " " + mediainfo_parts[1] + " and " + audio_to + bdinfo_audio_parts[2] + "\n")

      bdbitrate = bdinfo_audio_parts[4].strip()
      mbitrate = mediainfo_parts[3].strip()

      if bdbitrate == mbitrate:
        reply += self.print_report("error", "Audio " + self._audio_id(i) + ": " + audio_from + " " + bdinfo_audio_parts[2] + " to " + audio_to + " " + mediainfo_parts[1] + " same bitrate: " + str(bdbitrate) + "\n")
      else:
        reply += self.print_report("correct", "Audio " + self._audio_id(i) + ": " + audio_from + " " + bdinfo_audio_parts[2] + " to " + audio_to + " " + mediainfo_parts[1] + " (" + str(bdbitrate) + " to " + str(mbitrate) + ")\n")
    else:
      reply += self.print_report("error", "Audio " + self._audio_id(i) + " should be converted to " + audio_to + "\n")
      ...
      if 'title' in self.mediainfo['audio'][i]:
        ...
        if self._is_commentary_track(title):
          ...
          if len(matched_names) > 0:
            reply += self.print_report("correct", "Audio " + self._audio_id(i) + " Matched: " + ", ".join(matched_names) + "\n")
          unmatched_names = set(names) - set(matched_names)
          if len(unmatched_names) > 0:
            reply += self.print_report("warning", "Audio " + self._audio_id(i) + " Unmatched: " + ", ".join(unmatched_names) + "\n")
    ...
    for i, _ in enumerate(self.mediainfo['audio']):
      ...
      if 'title' in self.mediainfo['audio'][i]:
        ...
        if len(misspelled_words) > 0:
          reply += self.print_report("error", "Audio " + self._audio_id(i) + " Misspelled: " + ", ".join(misspelled_words) + "\n")
  ...
  def _audio_id(self, i):
    reply = ""
    if 'id' in self.mediainfo['audio'][i]:
      reply += "#" + self.mediainfo['audio'][i]['id']
    else:
      reply += str(i)
    return reply
